...
\begin{center}
\includegraphics[width=200pt, keepaspectratio]{licao01/barbantes-agnes.png}
\end{center}
...
\begin{center}
    %paralelogramo
\begin{tikzpicture}%[scale=20]
\filldraw [fill=stred]  (120:1) -- ++(180:1) -- ++(240:1) -- ++(0:2) -- ++(60:1) -- cycle;
\draw (180:1) --+ (60:1) -- (0,0);
\draw (180:1) --+ (120:1);
\end{tikzpicture}
\hspace{1cm}
...
\filldraw [fill=lightred] (0,0) -- (120:1) -- (180:1) -- cycle;
...
\end{tikzpicture}
\end{center}
      \item O triângulo cinza é um oitavo da figura azul, veja:
\begin{center}
      %Bandeirinha
...
\objetivos
  \begin{itemize} %d
    \item       Recompor a unidade a partir de uma fração unitária dada em modelos contínuos.
    \item       Relacionar uma fração da unidade à quantidade necessária dessas partes para compor a unidade. Assim, por exemplo, é necessário reunir cinco       {\itshape quintas partes}       para recompor a unidade.
\end{itemize} %d
